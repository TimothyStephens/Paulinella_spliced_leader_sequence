	 	...
	 	for win_start in range(start, (stop-region_size)+1):
			...
			for read in samfile.fetch(seqid, win_start, win_stop):
				...
				reference_end = read.reference_end      # aligned reference position of the read on the reference genome
									...
									# However in practice (through manual verification using IGV) it is clear that this is probabily not true and reference_end corresponds to the last aligned residue.
									# This is important because if its not giving us last_alinged_base + 1 then we end up with cases where the last window, if it hits the 3-prime end, will also ave 0 
									# passable reads and will always fail (becuase we expect the last read position to be seq_len + 1, which is simply not the case). 
